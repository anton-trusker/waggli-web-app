-- Phase 8: User Management - FORCE RECREATE
...
DO $$ BEGIN
  CREATE POLICY "Public read plans" ON subscription_plans FOR SELECT TO anon, authenticated USING (is_active = TRUE);
EXCEPTION WHEN duplicate_object THEN null; END $$;

DO $$ BEGIN
  CREATE POLICY "Users view own subscription" ON user_subscriptions FOR SELECT TO authenticated USING (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN null; END $$;

DO $$ BEGIN
  CREATE POLICY "Users manage own onboarding" ON onboarding_progress FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN null; END $$;
