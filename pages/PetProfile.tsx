...
import SharingTab from '../components/pet-profile/SharingTab';
import EditRecordModal from '../components/pet-profile/EditRecordModal';

interface PetProfileProps {
    onMenuClick?: () => void;
}

type TabType = 'Overview' | 'Passport' | 'Health Records' | 'Documents' | 'Team';

const PetProfile: React.FC<PetProfileProps> = ({ onMenuClick }) => {
    const { id } = useParams<{ id: string }>();
    const { pets, updatePet, updateVaccine, updateMedication, vaccines } = useApp();
    ...
    // handleSaveRecord removed as it is now handled within EditRecordModal
    ...
    const getAgeDisplay = () => {
        if (pet.birthday) {
            ...
            if (ageY > 0) return { val: ageY.toString(), unit: 'yrs' };
            if (ageM > 0) return { val: ageM.toString(), unit: 'mos' };
            const diffDays = Math.ceil(Math.abs(now.getTime() - birth.getTime()) / (1000 * 60 * 60 * 24));
            return { val: diffDays.toString(), unit: 'days' };
        }
        const val = (pet.age || '').replace(/[^\d.]/g, '') || '0';
        const unit = (pet.age || '').includes('m') ? 'mos' : 'yrs';
        return { val, unit };
    };

    const { val: ageValue, unit: ageUnit } = getAgeDisplay();

    // Weight Logic
    const getWeightDisplay = () => {
        const val = (pet.weight || '').replace(/[^\d.]/g, '') || '-';
        let unit = (pet.weight || '').replace(/[\d.\s]/g, '').toLowerCase();
        if (!unit && val !== '-') unit = 'kg'; // Default
        return { val, unit };
    };
    const { val: weightValue, unit: weightUnit } = getWeightDisplay();

    // Status Logic
    const getStatus = () => {
        const petVaccines = vaccines.filter(v => v.petId === pet.id);
        if (petVaccines.length === 0) return { label: 'Unknown', color: 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300', dot: 'bg-gray-400' };

        const overdue = petVaccines.some(v => v.status === 'Overdue');
        const expiring = petVaccines.some(v => v.status === 'Expiring Soon');

        if (overdue) return { label: 'Action Needed', color: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400', dot: 'bg-red-500' };
        if (expiring) return { label: 'Check-up Due', color: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400', dot: 'bg-yellow-500' };

        return { label: 'Healthy', color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400', dot: 'bg-green-500' };
    };
                ...
                <div className="relative mb-8">
                    ...
                    <div className="bg-surface-light dark:bg-surface-dark rounded-b-3xl shadow-sm border-x border-b border-gray-100 dark:border-gray-800 px-8 pb-6 pt-16 relative flex flex-col md:flex-row justify-between items-center md:items-end min-h-[140px]">
                        ...
                        <div className="absolute -top-16 left-8 md:left-10 p-1.5 bg-surface-light dark:bg-surface-dark rounded-full shadow-sm group">
                            ...
                            {/* Status Dot */}
                            <div className={`absolute bottom-3 right-3 w-7 h-7 border-4 border-surface-light dark:border-surface-dark rounded-full ${status.dot}`}></div>
                        </div>
                        ...
                        <div className="ml-0 md:ml-44 mt-16 md:mt-0 w-full md:w-auto text-center md:text-left mb-6 md:mb-1">
                            <div className="flex items-center justify-center md:justify-start gap-3 mb-1">
                                ...
                                <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide ${status.color}`}>{status.label} Status</span>
                            </div>
                    ...
                    <nav className="flex p-1.5 bg-gray-100 dark:bg-gray-800/50 rounded-xl w-full min-w-max md:w-auto md:inline-flex">
                        {tabItems.map(tab => (
                            <button
                                ...
                                className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 text-sm font-bold rounded-lg transition-all duration-200 whitespace-nowrap ${activeTab === tab.id
